  ...
  if (group.current && scrollProgress) {
    try {
      ...
      if (progress >= stopProgress) {
        progress = stopProgress; // Freeze at the end
        shouldReverse = false;
      }
      
      // Create looping effect after 0.55
      const maxProgress = 0.55;
      let loopedProgress = progress;
      let shouldReverse = false;
      let loopCount = 0;
      
      if (progress > maxProgress && progress < stopProgress) {
        shouldReverse = true;
      ...
      // Only apply transformations when scroll progress is greater than 0 AND less than stop progress
      if (progress > 0 && progress < stopProgress) {
        if (shouldReverse) {
          ...
          const targetX = -2;
          const targetY = 0; // Add this if missing
        ...
        // Forward tilt for flying position
        // group.current.rotation.x =
        // group.current.userData.initialRotation.x + Math.PI / 18;
